...
export class CandleCollector5M {
   ...
   * @param {Array} apiCandles - Raw API candles or pre-parsed candles
  ...
  transformCandles(apiCandles) {
    return apiCandles.map((candle, index) => {
      ...
      // Handle candles already parsed by the client (with timestamp field)
      if (candle.timestamp) {
        // timestamp could be a Date object or ISO string
        if (candle.timestamp instanceof Date) {
          timestamp = candle.timestamp.toISOString();
        } else if (typeof candle.timestamp === 'string') {
          timestamp = candle.timestamp;
        } else {
          logger.error('Invalid candle timestamp format', { candle, index });
          throw new Error(`Invalid timestamp format at index ${index}`);
        }
      }
      // Handle raw API format (with start field)
      else if (candle.start !== undefined) {
        if (typeof candle.start === 'string') {
          timestamp = candle.start;
        } else if (typeof candle.start === 'number') {
          timestamp = new Date(candle.start * 1000).toISOString();
        } else {
          logger.error('Invalid candle start field', { candle, index });
          throw new Error(`Invalid start field at index ${index}`);
        }
      }
      else {
        logger.error('Candle missing timestamp/start field', { candle, index });
        throw new Error(`Missing timestamp/start field at index ${index}`);
      }
  ...
  async backfill(count = DEFAULT_BACKFILL_COUNT) {
    ...
    // If count exceeds max per request, split into batches
    if (count > MAX_CANDLES_PER_REQUEST) {
      const batches = Math.ceil(count / MAX_CANDLES_PER_REQUEST);
      logger.info(`Splitting backfill into ${batches} batches`);

      for (let i = 0; i < batches; i++) {
        const batchSize = Math.min(MAX_CANDLES_PER_REQUEST, count - (i * MAX_CANDLES_PER_REQUEST));
        const minutesBack = ((i + 1) * MAX_CANDLES_PER_REQUEST) * 5;
        const batchEnd = now - (i * MAX_CANDLES_PER_REQUEST * 5 * 60 * 1000);
        const batchStart = batchEnd - (batchSize * 5 * 60 * 1000);

        logger.info(`Fetching batch ${i + 1}/${batches} (${batchSize} candles)`);

        const candles = await this.fetchCandles(batchStart, batchEnd);
        const transformed = this.transformCandles(candles);
        const results = await this.storeCandles(transformed);
    ...
    logger.info('Backfill complete', finalResults);
    return finalResults;
  }
  ...
  async collectLatest() {
    ...
    const closedCandles = transformed.filter(
