...
async function monitorPosition(tradeId, coinbaseClient) {
  ...
  try {
    ...
    if (progressToTarget >= 80 && !trade.trailing_stop_activated) {
      logger.info('Activating trailing stop at breakeven', {
        tradeId,
        progress: progressToTarget.toFixed(2) + '%'
      });

      await activateTrailingStop(trade, coinbaseClient);
...
/**
 * Activate trailing stop (move stop to breakeven)
 *
 * @param {Object} trade - Trade record
 * @param {Object} coinbaseClient - Coinbase API client
 * @private
 */
async function activateTrailingStop(trade, coinbaseClient) {
  await initializeDependencies();

  try {
    logger.info('Moving stop to breakeven', {
      tradeId: trade.id,
      oldStop: trade.stop_loss,
      newStop: trade.entry_price
    });

    // Determine new stop parameters
    ...
    // Update trade in database
    await dbQueries.updateTrade(trade.id, {
      stop_loss: trade.entry_price,
      coinbase_stop_order_id: newStopOrder.orderId,
      trailing_stop_activated: true,
      trailing_stop_time: new Date()
    });

    logger.info('Trailing stop activated', {
      tradeId: trade.id,
      newStopOrderId: newStopOrder.orderId
    });
  } catch (error) {
