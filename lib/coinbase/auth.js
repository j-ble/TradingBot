...
export async function generateJWT(apiKey, apiSecret, requestMethod, requestPath) {
  try {
    ...
    // Ensure key is in PKCS#8 format (required by CDP SDK for EC keys)
    try {
      if (privateKey.includes('BEGIN EC PRIVATE KEY')) {
        const keyObj = createPrivateKey(privateKey);
        privateKey = keyObj.export({
          format: 'pem',
          type: 'pkcs8'
        });
      }
    } catch (conversionError) {
      logger.warn('Failed to convert key to PKCS#8, using original', { error: conversionError.message });
      // Continue with original key, maybe it works or will fail later
    }
