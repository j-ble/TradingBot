...
export class CoinbaseClient {
  ...
  async request(method, path, data = null, options = {}) {
    ...
    // Build auth headers (async for JWT generation)
   ...
   * @param {string} granularity - Candle size ('5M', '1H', 'ONE_MINUTE', etc.)
  ...
  async getCandles(productId, granularity, start, end) {
    ...
    // Convert granularity shorthand to API format if needed
    let granularityValue = granularity;
    if (typeof granularity === 'string' && !granularity.includes('_')) {
      // It's a shorthand like '5M', convert to API format
      granularityValue = getGranularity(granularity);
    }
   ...
   * @returns {Promise<number>} - Current price (mid-point of bid/ask)
   */
  async getCurrentPrice(productId) {
    ...
    // Use best bid/ask for accurate current price
    const bidAsk = await this.getBestBidAsk([productId]);

    if (bidAsk.length > 0 && bidAsk[0].bids?.length > 0 && bidAsk[0].asks?.length > 0) {
      const bid = parseFloat(bidAsk[0].bids[0].price);
      const ask = parseFloat(bidAsk[0].asks[0].price);
      return (bid + ask) / 2; // Mid-point price
    }
    ...
    return parseFloat(response.trades?.[0]?.price || 0);
  }
   ...
   * @param {Array<string>} productIds - Array of product IDs (optional)
   * @returns {Promise<Object>} - Best bid/ask prices
   */
  async getBestBidAsk(productIds = null) {
    const params = {};
   ...
   * Close position
   * @param {string} productId - Product ID
   * @param {number} size - Position size to close (optional, closes full position if not specified)
   * @returns {Promise<Object>} - Close position response
   */
  async closePosition(productId, size = null) {
    if (!isValidProduct(productId)) {
      throw new Error(`Invalid product ID: ${productId}`);
    }
    ...
    if (size !== null) {
      closeData.size = size.toString();
    }

    logger.info('Closing position', { productId, size });

    return this.request('POST', ENDPOINTS.CLOSE_POSITION, closeData, { isOrder: true });
  }
   ...
   * @param {string} productId - Product ID
   * @param {string} side - 'BUY' or 'SELL'
   * @param {Object} orderConfiguration - Order configuration
   * @returns {Promise<Object>} - Order preview with estimated costs
   */
  async previewOrder(productId, side, orderConfiguration) {
    if (!isValidProduct(productId)) {
      throw new Error(`Invalid product ID: ${productId}`);
    }
    ...
    return this.request('POST', ENDPOINTS.PREVIEW_ORDER, previewData, { isOrder: true });
  }
  ...
  async listOrders(productId = null, status = null) {
    ...
    const response = await this.request('GET', ENDPOINTS.ORDERS_HISTORY, params);
